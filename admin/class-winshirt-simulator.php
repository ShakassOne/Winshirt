<?php
/**
 * WinShirt - Simulateur de Scénarios (BETA-2 failsafe)
 * Fichier: admin/class-winshirt-simulator.php
 *
 * - Page admin "Simulateur" (sous WinShirt si dispo, sinon top-level).
 * - Formulaire sympa (produits, lots, coûts) + calculs rentabilité.
 * - Prépa du lot TOUJOURS incluse (tirage ET remboursement).
 * - Graphique Chart.js.
 * - Sauvegarde/chargement des scénarios via CPT `ws_scenario`.
 */
...
class WinShirt_Simulator {
    ...
    public function add_menu() {
        // Essaie de se mettre sous le menu parent "WinShirt".
        $parent_slug = 'winshirt';
        ...
        // Si le parent n’existe pas, crée un top-level minimal + le sous-menu.
        if ( empty($added) ) {
            add_menu_page('WinShirt','WinShirt','manage_options',$parent_slug,function(){
                echo '<div class="wrap"><h1>WinShirt</h1><p>Tableau de bord WinShirt</p></div>';
            }, 'dashicons-tickets', 56);
            add_submenu_page(
                $parent_slug,'Simulateur','Simulateur','manage_options',self::MENU_SLUG,[$this,'render_admin_page']
            );
        }
    }
    ...
    public function enqueue($hook) {
        ...
        // Media uploader (images produits/lots)
        wp_enqueue_media();
        ...
        // JS utilitaires pour duplications et media
        $js = '
        (function($){
    ...
    private function f($v,$def=0.0){ if(is_string($v)) $v=str_replace(',','.',$v); return is_numeric($v)?(float)$v:(float)$def; }

    private function fetch_config_from_post() : array {
        $d = $this->defaults(); $p = wp_unslash($_POST); $c = [];
        $c['scenario_title']   = sanitize_text_field($p['scenario_title'] ?? '');
        $c['save_scenario']    = !empty($p['save_scenario']) ? 1 : 0;
        $c['tva']              = $this->f($p['tva'] ?? $d['tva'])/ ( (isset($p['tva']) && $p['tva']>1) ? 100 : 1 );
        $c['shipping_ht']      = $this->f($p['shipping_ht'] ?? $d['shipping_ht']);
        $c['refund_amount_ht'] = $this->f($p['refund_amount_ht'] ?? $d['refund_amount_ht']);
        $c['refund_enabled']   = !empty($p['refund_enabled']) ? 1 : 0;
        $c['draw_threshold']   = (int)($p['draw_threshold'] ?? $d['draw_threshold']);
        $c['sacs_total_ht']    = $this->f($p['sacs_total_ht'] ?? $d['sacs_total_ht']);
        $c['fix_structure']    = $this->f($p['fix_structure'] ?? $d['fix_structure']);
        $c['fix_com']          = $this->f($p['fix_com'] ?? $d['fix_com']);
        $c['fix_huissier']     = $this->f($p['fix_huissier'] ?? $d['fix_huissier']);
        $c['fix_divers']       = $this->f($p['fix_divers'] ?? $d['fix_divers']);
        $c['external_ref_area']= $this->f($p['external_ref_area'] ?? $d['external_ref_area']);
        $c['external_ref_cost']= $this->f($p['external_ref_cost'] ?? $d['external_ref_cost']);
        ...
        if ( ! empty($p['prod_name']) && is_array($p['prod_name']) ) {
            ...
            for ($i=0;$i<$count;$i++){
                $c['products'][] = [
                    ...
                    'price_ttc'         => $this->f($p['prod_price_ttc'][$i] ?? 0),
                    'cost_garment_ht'   => $this->f($p['prod_cost_vet_ht'][$i] ?? 0),
                    'area_cm2'          => $this->f($p['prod_area_cm2'][$i] ?? 0),
                    'cost_internal_cm2' => $this->f($p['prod_cost_int_cm2'][$i] ?? 0),
                    'cost_external_cm2' => $this->f($p['prod_cost_ext_cm2'][$i] ?? 0),
                    'use_internal'      => !empty($p['prod_use_internal'][$i]) ? 1 : 0,
                    'shipping_unit_ht'  => $this->f($p['prod_ship_ht'][$i] ?? $c['shipping_ht']),
                    'mix'               => $this->f($p['prod_mix'][$i] ?? 0),
        ...
        if ( ! empty($p['prize_name']) && is_array($p['prize_name']) ) {
            ...
            for ($i=0;$i<$count;$i++){
                $c['prizes'][] = [
                    ...
                    'value_ht' => $this->f($p['prize_value_ht'][$i] ?? 0),
                    'prep_ht'  => $this->f($p['prize_prep_ht'][$i] ?? 0),
    ...
    private function save_scenario(array $cfg, array $results) : int {
        ...
        if (empty($title)) $title = 'Scénario ' . current_time('Y-m-d H:i');
    ...
    private function ext_cm2_default(array $cfg) : float {
    ...
    private function calc_results(array $cfg) : array {
        ...
        // Normaliser mix
        $mix_sum = 0.0; foreach ($cfg['products'] as $p) { $mix_sum += (float)$p['mix']; }
        ...
        foreach ($cfg['volumes'] as $volume) {
            ...
            foreach ($cfg['prizes'] as $prize) {
                ...
                $prep  = (float)$prize['prep_ht']; // TOUJOURS incluse (ton choix)
                ...
                $res[] = ['volume'=>$volume,'prize'=>$name,'mode'=>'Tirage – Interne','ca_ht'=>$ca_ht,'var'=>$var_total_internal,'fixed'=>$fixed_non_prize,'prep'=>$prep,'lot'=>$value,'refund'=>0.0,'profit'=>$profit_internal_draw];
                $res[] = ['volume'=>$volume,'prize'=>$name,'mode'=>'Remboursement – Interne','ca_ht'=>$ca_ht,'var'=>$var_total_internal,'fixed'=>$fixed_non_prize,'prep'=>$prep,'lot'=>0.0,'refund'=>$refund_total,'profit'=>$profit_internal_refund];
                $res[] = ['volume'=>$volume,'prize'=>$name,'mode'=>'Tirage – Externe','ca_ht'=>$ca_ht,'var'=>$var_total_external,'fixed'=>$fixed_non_prize,'prep'=>$prep,'lot'=>$value,'refund'=>0.0,'profit'=>$profit_external_draw];
                $res[] = ['volume'=>$volume,'prize'=>$name,'mode'=>'Remboursement – Externe','ca_ht'=>$ca_ht,'var'=>$var_total_external,'fixed'=>$fixed_non_prize,'prep'=>$prep,'lot'=>0.0,'refund'=>$refund_total,'profit'=>$profit_external_refund];
            }
        }
    ...
    public function render_admin_page() {
        ...
        if ( isset($_POST['ws_simulator_submit']) ) {
            ...
            if ( ! empty($cfg['save_scenario']) ) {
                ...
                if ($saved_id) $notice = '<div class="ws-card" style="border-color:#10b981"><strong>Scénario enregistré</strong> (ID: '.(int)$saved_id.')</div>';
            }
        } elseif ( $loaded_id ) {
        ...
        $existing = get_posts(['post_type'=>self::CPT,'posts_per_page'=>20,'orderby'=>'date','order'=>'DESC']);
        ...
        echo '<div class="ws-col-6" style="display:flex;align-items:flex-end;gap:12px;">
                <label class="ws-flex"><input type="checkbox" name="save_scenario" value="1" '.checked(1,$cfg['save_scenario'],false).' class="ws-checkbox">Sauvegarder</label>
              </div>';
        echo '</div></div>';
        ...
        echo '<div class="ws-col-12" style="margin-top:6px;"><label class="ws-flex"><input type="checkbox" name="refund_enabled" value="1" '.checked(1,$cfg['refund_enabled'],false).' class="ws-checkbox">Activer le remboursement si ventes &lt; seuil</label></div>';
        echo '</div></div>';
    ...
    private function render_prize_row($pr=null, $is_template=false) {
        $d = [ 'name'=>'Nouveau lot','value_ht'=>0.0,'prep_ht'=>0.0,'image'=>'' ];
    ...
    private function render_results(array $rows) {
        ...
        foreach (['Volume','Lot','Mode','CA HT','Coût variables','Fixes (hors lot)','Prépa','Lot','Remboursements','Résultat HT'] as $h)
    ...
    private function build_chart_dataset(array $rows) : array {
        ...
        foreach ($rows as $r) {
            ...
            $k = $r['volume'];
            if ( ! isset($dataset[$k]) ) $dataset[$k] = ['draw'=>null,'refund'=>null];
    ...
    private function render_chart(array $chart) {
        ...
        $labels=[]; $draw=[]; $refund=[];
        foreach ($chart as $vol=>$vals){ $labels[]=$vol; $draw[]=round($vals['draw']??0,2); $refund[]=round($vals['refund']??0,2); }
