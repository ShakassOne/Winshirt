...
/**
 * CPT Loteries + shortcodes + formulaire (overlay au survol).
 *
 * - Cartes full-image avec overlay dégradé noir→transparent qui slide au hover/focus.
 ...
 *
 * Remarques robustesse :
 * - Tous les appels aux fonctions WordPress sont préfixés par "\" (espace global) pour éviter
 *   toute résolution de nom en espace WinShirt.
 * - Pas de types scalaires dans les signatures publiques côté hooks pour éviter des fatals
 *   si des extensions tierces hookent avec des types différents.
 */
class Lottery {
    ...
    public function mb_participants(\WP_Post $post) {
        ...
        <script>
        (function(){
            var btn = document.getElementById('ws-export-csv');
            if(!btn) return;
            btn.addEventListener('click', function(e){
                e.preventDefault();
                var id = this.getAttribute('data-id') || '0';
                var url = (window.ajaxurl || '<?php echo \esc_js(\admin_url('admin-ajax.php')); ?>') +
                          '?action=ws_lottery_export_csv&post_id=' + encodeURIComponent(id) +
                          '&_wpnonce=<?php echo \wp_create_nonce('ws_lottery_csv'); ?>';
     ...
     * HELPERS
     * ============================================================ */
    private function fmt_dt_local($stored) {
        if(empty($stored)) return '';
        $ts=\strtotime($stored); return $ts?\date_i18n('Y-m-d\TH:i',$ts):'';
    }
    private function parse_dt_local($v) {
        if(empty($v)) return '';
        $ts=\strtotime($v); return $ts?\date_i18n('Y-m-d H:i:s',$ts):'';
    }
    ...
    public function inline_css() { ?>
        <style id="winshirt-lottery-cards">
            ...
            .ws-card{position:relative; border-radius:16px; overflow:hidden; box-shadow:0 6px 20px rgba(0,0,0,.10); outline:none}
            .ws-card:focus{box-shadow:0 0 0 3px rgba(99,102,241,.5), 0 6px 20px rgba(0,0,0,.10)}
            .ws-card-link{display:block; color:inherit; text-decoration:none}
            .ws-card-figure{position:relative; margin:0; aspect-ratio: 16 / 9; background:#0a0a0a}
            .ws-card-img{width:100%; height:100%; object-fit:cover; display:block}
            .ws-card-ph{background:linear-gradient(135deg,#111,#1f1f1f)}
            .ws-badge{position:absolute;top:12px;left:12px;padding:6px 10px;border-radius:999px;background:#6d28d9;color:#fff;font-size:12px;font-weight:600}
            ...
            .ws-card:focus .ws-card-overlay,
            .ws-card:focus-within .ws-card-overlay{ transform: translateY(0%); }

            .ws-card-title{margin:0 0 2px; font-size:18px; line-height:1.25; font-weight:700}
            .ws-line{margin:0; font-size:14px; color:#E6E6E6}
            .ws-timer{font-family:ui-monospace,monospace}
            .ws-btn-ghost{
                align-self:flex-start; margin-top:4px; border:1px solid rgba(255,255,255,.9);
                color:#fff; padding:8px 12px; border-radius:8px; font-weight:600; font-size:14px; background:transparent;
            }
            ...
            @media (max-width: 640px){
                ...
                .ws-lottery-list.ws-slider > .ws-card{min-width:85vw}
            }
        </style>
    <?php }
}
