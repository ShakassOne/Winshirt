<?php
/**
 * Template single loterie (carte large + métriques + formulaire).
 */
...
<main class="ws-wrap">
  <article class="ws-hero">
    <div class="ws-hero-media">
      <?php echo $thumb ?: '<div class="ws-hero-ph"></div>'; ?>
    ...
    <div class="ws-hero-bottom">
      ...
      <div class="ws-draw">
        ...
        <?php if ($over): ?>
          <button class="ws-btn ws-btn-disabled" disabled><?php esc_html_e('Terminé','winshirt'); ?></button>
        <?php else: ?>
          <a class="ws-btn" href="#ws-form"><?php esc_html_e('Participer','winshirt'); ?></a>
        <?php endif; ?>
      </div>
    </div>
  </article>
  ...
  <section class="ws-grid-2">
    ...
    <aside id="ws-form" class="card">
      <?php if ($over): ?>
        <div class="ws-ended"><?php esc_html_e('Cette loterie est terminée.','winshirt'); ?></div>
      <?php else: ?>
        <?php echo do_shortcode('[winshirt_lottery_form id="'.$id.'"]'); ?>
      <?php endif; ?>
    </aside>
  </section>
</main>
